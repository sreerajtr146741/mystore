...
<main class="container py-4">
  ...
  @if($errors->any())
    <div class="alert alert-danger alert-dismissible fade show">
      <div class="fw-semibold mb-1"><i class="bi bi-exclamation-triangle me-1"></i>There were some problems:</div>
      <ul class="mb-0">
  ...
  <div class="table-responsive">
    <table class="table table-dark table-striped align-middle">
      ...
      @forelse($products as $p)
        @php
          // If you added a final_price accessor use it; else compute here safely
          $base = (float)($p->price ?? 0);
          $final = $base;
          $now = now();
          $inWindow = ($p->discount_starts_at && $p->discount_ends_at)
            ? $now->between($p->discount_starts_at, $p->discount_ends_at)
            : true;
          if (($p->is_discount_active ?? false) && $p->discount_type && $p->discount_value && $inWindow) {
              $final = $p->discount_type === 'percent'
                ? max(0, $base - ($base * ((float)$p->discount_value/100)))
                : max(0, $base - (float)$p->discount_value);
          }
        @endphp
        <tr>
          <td>{{ $p->id }}</td>
          ...
          <td class="text-end">
            ...
            <button class="btn btn-sm btn-outline-primary"
               ...
               data-discount_active="{{ $p->is_discount_active ? 1 : 0 }}"
            >
      ...
      <div class="modal-body">
        ...
        <div class="row">
          <div class="col-md-6 mb-3">
            ...
            <input name="discount_starts_at" type="datetime-local" class="form-control">
          </div>
          <div class="col-md-6 mb-3">
            <label class="form-label">Discount Ends</label>
            <input name="discount_ends_at" type="datetime-local" class="form-control">
          </div>
        </div>
        <div class="form-check mb-2">
          <input class="form-check-input" type="checkbox" name="is_discount_active" value="1" id="discActiveCreate">
